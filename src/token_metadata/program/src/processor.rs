...
pub fn process_create_metadata_accounts_v2<'a>(
    ...
    let account_info_iter = &mut accounts.iter(); // metadata_account, mint, mint_authority, payer, update_authority, system_program_id, rent_program_id를 담은 배열
    let metadata_account_info = next_account_info(account_info_iter)?;
    let mint_info = next_account_info(account_info_iter)?;
    let mint_authority_info = next_account_info(account_info_iter)?; // 이 nft  구매자의 정보
    let payer_account_info = next_account_info(account_info_iter)?;
    let update_authority_info = next_account_info(account_info_iter)?; // 구매자 정보
    let system_account_info = next_account_info(account_info_iter)?;
    let rent_info = next_account_info(account_info_iter)?;  // 자식 nft 정보를 받아야 한다.
